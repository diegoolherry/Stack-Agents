...
6. Si el proyecto usa Supabase como DB, leer `.opencode/skills/supabase/SKILL.md` y documentar en architecture.md: setup requerido, y agregar `supabase/migrations/**` como path sensible en security-trigger.config.json (además de auth/db)
7. Leer `.opencode/skills/deploy/SKILL.md` y generar `.github/workflows/ci.yml` (+ deploy si aplica) como parte del setup inicial del proyecto
